...
def indexCreateProgram(configFile):
    ...
    if result["code"]==0:
        ...
        try:
            ...
            config_query_set = AlgorithmConfigInfo.objects.filter(config_name=configFile[len(CONFIGFILEPATH_ABS):])
...
def indexSearchProgram(search_string, configFile):
    ...
    if result["code"]==0:
        ...
        close_old_connections()
        config_info = AlgorithmConfigInfo.objects.filter(config_name=configFile[len(CONFIGFILEPATH_ABS):])[0]
        # except Exception, e:
